...
from msynth.expr_utils import get_unification_candidates, get_subexpressions, get_unique_variables
from msynth.oracle import InputOutputOracle
...
class Simplifier:
    """
    Expression simplification based on a pre-computed synthesis oracle.

    The Simplifier has access to a pre-computed synthesis oracle, stores
    ...
    Attributes:
        oracle (InputOutputOracle): Pre-computed synthesis oracle.
    ...
    def __init__(self, oracle_path: Path, enforce_equivalence: bool = False, solver_timeout: int = 1):
        ...
        Args:
            oracle_path: File path to pre-computed synthesis oracle.
        ...
        """
        # public attributes
        self.oracle = InputOutputOracle.load_from_file(oracle_path)
    ...
    @staticmethod
    def _gen_unification_dict(expr: Expr) -> Dict[Expr, Expr]:
        """
    ...
    def determine_equivalence_class(self, expr: Expr) -> str:
        ...
        expression's output behavior and query the synthesis
        oracle.
    ...
    def _reverse_unification(self, expr: Expr, unification_dict: Dict[Expr, Expr]) -> Expr:
        """
    ...
    def _is_suitable_simplification_candidate(self, expr: Expr, simplified: Expr) -> bool:
        ...
        3. If Miasm's expression simplification results in the same expression for
           ...
           synthesis oracle is not required.
        ...
        if any([re.search("^p[0-9]*", v.name) for v in get_unique_variables(simplified)]):
            logger.debug(f"{expr} <==> {simplified} (incorrect variable replacement)")
            return False
        ...
        if self.enforce_equivalence and self.check_semantical_equivalence(expr, simplified) != z3.unsat:
            logger.debug(f"{expr} <==> {simplified} (not semantically equivalent)")
            return False
        # SMT solver finds a counter example
        if self.check_semantical_equivalence(expr, simplified) == z3.sat:
            logger.debug(f"{expr} <==> {simplified} (not semantically equivalent, counterexample found)")
            return False
        return True
    ...
    def _find_suitable_simplification(self, equiv_class: str, expr: Expr, unification_dict: Dict[Expr, Expr]) -> Tuple[bool, Expr]:
        ...
        for candidate in self.oracle.get_equiv_class_members(equiv_class):
            ...
            simplified = self._reverse_unification(candidate, unification_dict)
    ...
    def simplify(self, expr: Expr) -> Expr:
        ...
        while True:
            ...
            for subtree in get_subexpressions(ast):
                ...
                unification_dict = Simplifier._gen_unification_dict(subtree)
