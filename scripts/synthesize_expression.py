...
def mba(size: int) -> Expr:
    ...
    v1 = ExprId("v1", size)
    v2 = ExprId("v2", size)
    return (((v1 ^ v2) + ((v1 & v2) << ExprInt(1, size))) | v0) + (
        ((v1 ^ v2) + ((v1 & v2) << ExprInt(1, size))) & v0
    )
...
def main(expr: Expr, num_io_samples: int) -> None:
    ...
    synthesized, score = simplifier.synthesize_from_expression(
        expr, num_samples=num_io_samples
    )
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Simplify expressions (especially MBAs) using stochastic program synthesis"
    )
    parser.add_argument(
        "--samples",
        dest="num_io_samples",
