...
def main(
    ...
    logger.info(
        f"Computing oracle for {num_variables} variables and {num_io_samples} samples. Using library at '{library_path.as_posix()}'"
    )
    oracle = SimplificationOracle(num_variables, num_io_samples, library_path)
    logger.info(
        f"Writing oracle to {oracle_path.as_posix()} (format: {'sqlite' if use_sqlite else 'pickle'})"
    )
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Precompute SimplificationOracle which then can be used to simplify MBA expressions"
    )
    parser.add_argument(
        "library_path",
        type=Path,
        help="Path to a pre-computed set of expressions that shall be clustered in a oracle",
    )
    parser.add_argument(
        "oracle_path", type=Path, help="Output path to store the calculated oracle"
    )
    parser.add_argument(
        "--variables",
        dest="num_variables",
